    ...
    if (headquarter && type && date_init && date_end) {
      result = await connection.query(
        `SELECT (select s.tipo from servicios s where s.id = r.servicios_id) as tipo, r.fecha_hora_inicio_reserva, r.fecha_hora_fin_reserva,  r.motivo_reserva,  r.fecha_registro ,
         v.valoracion, v.comentario_valoracion,v.fecha_registro FROM reservas r INNER JOIN valoraciones v ON v.incidencias_id=r.id  WHERE r.usuarios_id IN (SELECT u.id FROM usuarios u WHERE u.sedes_id IN 
      ...
      result = await connection.query(
        `SELECT (select s.tipo from servicios s where s.id = r.servicios_id) as tipo,  r.fecha_hora_inicio_reserva, r.fecha_hora_fin_reserva,  r.motivo_reserva,  r.fecha_registro ,
         v.valoracion, v.comentario_valoracion,v.fecha_registro FROM reservas r INNER JOIN valoraciones v ON v.incidencias_id=r.id  WHERE (r.servicios_id = (SELECT s.id FROM servicios s WHERE s.tipo LIKE ?))
      ...
      result = await connection.query(
        `SELECT (select s.tipo from servicios s where s.id = r.servicios_id) as tipo,  r.fecha_hora_inicio_reserva, r.fecha_hora_fin_reserva,  r.motivo_reserva,  r.fecha_registro ,
         v.valoracion, v.comentario_valoracion,v.fecha_registro FROM reservas r INNER JOIN valoraciones v ON v.incidencias_id=r.id   WHERE (r.servicios_id = (SELECT s.id FROM servicios s WHERE s.tipo LIKE ?)) ORDER BY r.fecha_hora_inicio_reserva DESC;`,
      ...
      result = await connection.query(
        `SELECT (select s.tipo from servicios s where s.id = r.servicios_id) as tipo,   r.fecha_hora_inicio_reserva, r.fecha_hora_fin_reserva,  r.motivo_reserva,  r.fecha_registro ,
         v.valoracion, v.comentario_valoracion,v.fecha_registro FROM reservas r INNER JOIN valoraciones v ON v.incidencias_id=r.id  WHERE (r.fecha_hora_inicio_reserva >=  ? AND  r.fecha_hora_inicio_reserva <  ?) ORDER BY r.fecha_hora_inicio_reserva DESC;`,
        [
          formatDateToDB(new Date(date_init)),
          formatDateToDB(new Date(date_end))
        ]
      ...
      result = await connection.query(
        `SELECT (select s.tipo from servicios s where s.id = r.servicios_id) as tipo,  r.fecha_hora_inicio_reserva, r.fecha_hora_fin_reserva,  r.motivo_reserva,  r.fecha_registro ,
         v.valoracion, v.comentario_valoracion,v.fecha_registro FROM reservas r INNER JOIN valoraciones v ON v.incidencias_id=r.id  ORDER BY r.fecha_hora_inicio_reserva DESC;`
      );
    }
